...
**Phase 3 — Harden and complete: BUILT (9 Jul 2026 — platform #59, billieChat #135, billie-crm #56, tooling #12, SDKs 0.3.1/2.10.0/2.3.0):**
WhatsApp channel support end-to-end (ClickSend WhatsApp client, dispatcher branch, templates, channel_preference honoured) — env-gated dark until Meta template approval + sender provisioning (`marketing_whatsapp_enabled`); DSR complete (erase now XDELs the marketing event stream — tombstone kept, chatLedger ages out via its 10k trim — plus `ExportContact` subject-access export, admin-gated in the CRM with the audit trail merged in); one-click CSV exports (grid + feedback queue); audit hardening review done (append-only + ids-only verified; consent `method` stripped on erasure); restore drill now asserts `marketing.*` + `customer.customer_state` survive (billie-platform-tooling); `SETTLED_SHORT` in accounts SDK 2.10.0 (state projection maps it to C-N). Also landed: batch send outcomes (`batch.invitations.triggered.v1` → invited_at + counts — fixing the never-written `invited_at` that left the "invited" stage derivation dead) and the collections↔marketing suppression bridge (stop-contact raises the dispatcher kill-switch; marketing dispatches carry linked customer_id so the gate actually applies).
