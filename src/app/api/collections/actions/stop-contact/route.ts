/**
 * API Route: POST /api/collections/actions/stop-contact
 *
 * Operator action — applies a stop-contact flag to a collections case
 * (e.g. dispute, deceased, legal). Synchronous gRPC command against the
 * headless collections engine (BTB-198 WS5); the engine applies the FSM
 * transition, emits `collection.case.*` to ChatLedger, and returns a
 * verdict.
 *
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await getCollectionsServiceClient().applyStopContact({
